...
<div class="p-6">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 p-6 mb-6">
        ...
        <div class="flex items-center justify-between mt-4 pt-4 border-t border-gray-100">
            ...
            <div class="flex items-center space-x-2">
                ...
                <button id="export-btn" class="text-sm bg-gray-100 hover:bg-gray-200 px-3 py-2 rounded-lg transition-colors">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        ...
        <div id="items-grid" class="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            <div class="group bg-white border border-gray-200 rounded-2xl overflow-hidden hover:shadow-lg hover:border-amerta-primary/30 transition-all duration-300">
                <!-- Item Thumbnail -->
                <div class="relative h-48 bg-gradient-to-br 
                    {{ $item->category === 'dokumen' ? 'from-blue-500 to-blue-600' : 
                       ($item->category === 'foto' ? 'from-pink-500 to-pink-600' : 
                       ($item->category === 'video' ? 'from-purple-500 to-purple-600' : 
                       ($item->category === 'audio' ? 'from-orange-500 to-orange-600' : 
                       'from-gray-400 to-gray-500'))) 
                    }} flex items-center justify-center">
                    <i class=""
                        {{ $item->category === 'dokumen' ? 'fas fa-file-pdf' : 
                           ($item->category === 'foto' ? 'fas fa-images' : 
                           ($item->category === 'video' ? 'fas fa-video' : 
                           ($item->category === 'audio' ? 'fas fa-music' : 
                           'fas fa-file'))) 
                        }} text-4xl text-white"></i>
                    <div class="absolute top-3 left-3">
                        <span class="
                            {{ $item->category === 'dokumen' ? 'bg-blue-100 text-blue-800' : 
                               ($item->category === 'foto' ? 'bg-pink-100 text-pink-800' : 
                               ($item->category === 'video' ? 'bg-purple-100 text-purple-800' : 
                               ($item->category === 'audio' ? 'bg-orange-100 text-orange-800' : 
                               'bg-gray-100 text-gray-800'))) 
                            }} text-xs font-medium px-2 py-1 rounded-full">
                            {{ ucfirst($item->category) }}
                        </span>
                    </div>
                    {{-- Tambahkan preview/thumbnail jika ada --}}
                    @if($item->thumbnail_url)
                        <img src="{{ $item->thumbnail_url }}" alt="Thumbnail" class="absolute inset-0 w-full h-full object-cover opacity-60">
                    @endif
                </div>
                ...
                <div class="p-4">
                    <div class="flex items-start justify-between mb-2">
                        ...
                        <div class="ml-2 flex items-center space-x-1">
                            @if($item->file_url)
                            <a href="{{ $item->file_url }}" class="p-1 text-gray-400 hover:text-blue-600 transition-colors" title="Download" target="_blank">
                                <i class="fas fa-download text-sm"></i>
                            </a>
                            @endif
                            <button class="p-1 text-gray-400 hover:text-green-600 transition-colors btn-preview" title="Preview" data-id="{{ $item->id }}">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center space-x-1">
                            <button class="p-1 text-gray-400 hover:text-amerta-primary transition-colors btn-edit" title="Edit" data-id="{{ $item->id }}">
                                <i class="fas fa-edit text-sm"></i>
                            </button>
                            <button class="p-1 text-gray-400 hover:text-red-600 transition-colors btn-delete" title="Hapus" data-id="{{ $item->id }}">
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden">
        ...
        <div class="px-6 py-4 border-b border-gray-100 bg-gray-50">
            <div class="flex items-center justify-between">
                ...
                <button id="close-item-modal" class="p-2 text-gray-400 hover:text-gray-600 rounded-lg transition-colors">
        ...
        <div class="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
            <form action="/create/item" method="POST" id="item-form" class="space-y-6">
                ...
                <div>
                    ...
                    <div class="border-2 border-dashed border-gray-200 rounded-xl p-6 text-center hover:border-amerta-primary/50 transition-colors">
                        <input type="file" id="item-file" name="file" class="hidden" accept=".pdf,.jpg,.jpeg,.png,.mp4,.mp3,.wav">
                        ...
                        <div id="file-preview" class="hidden">
                            <div class="flex items-center justify-center space-x-3 bg-gray-50 rounded-lg p-4">
                                ...
                                <button type="button" id="remove-file" class="p-1 text-red-500 hover:bg-red-50 rounded">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-1">Format JSON untuk informasi tambahan seperti tags, tahun, sumber, dll.</p>
                </div>
...
<!-- Bulk Upload Modal -->
<div id="bulk-upload-modal" class="fixed inset-0 z-50 hidden bg-black bg-opacity-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl">
        <!-- Modal Header -->
        <div class="px-6 py-4 border-b border-gray-100 bg-gray-50">
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-bold text-gray-800">Upload Batch</h2>
                <button id="close-bulk-modal" class="p-2 text-gray-400 hover:text-gray-600 rounded-lg transition-colors">
                    <i class="fas fa-times text-xl"></i>
                </button>
            </div>
        </div>
        ...
        <div class="p-6">
            ...
            <!-- Bulk Upload Area -->
            <div class="border-2 border-dashed border-gray-200 rounded-xl p-8 text-center hover:border-amerta-primary/50 transition-colors mb-6">
            ...
            <div id="upload-progress" class="hidden">
                <div class="bg-gray-50 rounded-xl p-4 mb-4">
                    ...
                    <div class="w-full bg-gray-200 rounded-full h-2">
                        <div class="bg-amerta-primary h-2 rounded-full transition-all duration-300" style="width: 60%"></div>
                    </div>
                </div>
            </div>
        ...
        <div class="px-6 py-4 border-t border-gray-100 bg-gray-50 flex items-center justify-end space-x-3">
            ...
            <button id="start-bulk-upload" type="button"
                class="px-6 py-3 bg-amerta-primary hover:bg-amerta-dark text-white rounded-xl transition-colors disabled:opacity-50" disabled>
...
<div id="delete-item-modal" class="fixed inset-0 z-50 hidden bg-black bg-opacity-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        <div class="p-6 text-center">
            ...
            <p class="text-gray-600 mb-6">Apakah Anda yakin ingin menghapus item ini? File dan data tidak dapat dikembalikan.</p>
            <div class="flex items-center justify-center space-x-3">
                <button id="cancel-delete-item" 
                    class="px-4 py-2 text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors">
                    Batal
                </button>
                <button id="confirm-delete-item" 
...
<div id="preview-modal" class="fixed inset-0 z-50 hidden bg-black bg-opacity-90 flex items-center justify-center p-4">
    <div class="relative w-full max-w-6xl max-h-[90vh]">
        <button id="close-preview" class="absolute top-4 right-4 z-10 p-2 bg-white/20 hover:bg-white/30 text-white rounded-lg">
            <i class="fas fa-times text-xl"></i>
        </button>
...
<script>
    ...
    const categoryIcons = {
        'dokumen': { icon: 'fas fa-file-pdf', color: 'from-blue-500 to-blue-600', badge: 'bg-blue-100 text-blue-800' },
        'foto': { icon: 'fas fa-images', color: 'from-pink-500 to-pink-600', badge: 'bg-pink-100 text-pink-800' },
        'video': { icon: 'fas fa-video', color: 'from-purple-500 to-purple-600', badge: 'bg-purple-100 text-purple-800' },
        'audio': { icon: 'fas fa-music', color: 'from-orange-500 to-orange-600', badge: 'bg-orange-100 text-orange-800' },
        'artefak': { icon: 'fas fa-gem', color: 'from-emerald-500 to-emerald-600', badge: 'bg-emerald-100 text-emerald-800' },
        'cerita_rakyat': { icon: 'fas fa-book-open', color: 'from-amber-500 to-amber-600', badge: 'bg-amber-100 text-amber-800' },
        'resep_tradisional': { icon: 'fas fa-utensils', color: 'from-red-500 to-red-600', badge: 'bg-red-100 text-red-800' }
    };
    ...
    function updateQuery() {
        ...
        if (uploader) params.set('uploaded_by', uploader); else params.delete('uploaded_by');
    ...
    document.getElementById('confirm-delete-item').addEventListener('click', function() {
        if (deleteId) {
            ...
            window.location.href = '/items/' + deleteId + '/delete';
        }
    });
